...
//箭头所在方向
typedef NS_ENUM(NSUInteger, LeePopupContainerViewLayoutDirection) {
    LeePopupContainerViewLayoutDirectionAbove,
    LeePopupContainerViewLayoutDirectionBelow
};

@interface LeePopupContainerView : UIControl {
...
/// 计算布局时期望的默认位置，默认为LeePopupContainerViewLayoutDirectionAbove，也即在目标的上方
@property(nonatomic, assign) LeePopupContainerViewLayoutDirection preferLayoutDirection UI_APPEARANCE_SELECTOR;

/// 最终的布局方向（preferLayoutDirection只是期望的方向，但有可能那个方向已经没有剩余空间可摆放控件了，所以会自动变换）
@property(nonatomic, assign, readonly) LeePopupContainerViewLayoutDirection currentLayoutDirection;
...
@interface LeePopupContainerView (UISubclassingHooks)
