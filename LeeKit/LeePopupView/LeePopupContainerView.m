...
#import "LeePopupContainerView.h"
#import "UIControl+Lee.h"

@interface LeePopupContainerViewWindow: UIWindow

@end

@implementation LeePopupContainerViewWindow
...
@interface LeePopContainerMaskControl : UIControl

@property(nonatomic, weak) LeePopupContainerView *popupContainerView;
@end



@interface LeePopContainerViewController : UIViewController

@end

@implementation LeePopContainerViewController

- (void)loadView {
    LeePopContainerMaskControl *maskControl = [[LeePopContainerMaskControl alloc] init];
    self.view = maskControl;
}

@end

@interface LeePopupContainerView (UIAppearance)

- (void)updateAppearance;
@end

@interface LeePopupContainerView () {
    UIImageView                     *_imageView;
    UILabel                         *_textLabel;
}

@property(nonatomic, strong) LeePopupContainerViewWindow *popupWindow;
@property(nonatomic, weak) UIWindow *previousKeyWindow;
@property(nonatomic, assign) BOOL hidesByUserTap;
@end

@implementation LeePopupContainerView
...
- (CGSize)sizeThatFits:(CGSize)size {
    size.width = fmin(size.width, CGRectGetWidth(self.superviewIfExist.bounds) - self.safetyMarginsOfSuperview.left - self.safetyMarginsOfSuperview.right);
    size.height = fmin(size.height, CGRectGetHeight(self.superviewIfExist.bounds) - self.safetyMarginsOfSuperview.top - self.safetyMarginsOfSuperview.bottom);
    //内部内容的大小，去掉边框，箭头等
...
-(void)layoutSubviews {
    ...
    CGRect roundedRect = CGRectMake(self.borderWidth / 2.0, self.borderWidth / 2.0 + (self.currentLayoutDirection == LeePopupContainerViewLayoutDirectionAbove ? 0 : arrowSize.height), CGRectGetWidth(self.bounds) - self.borderWidth, CGRectGetHeight(self.bounds) - arrowSize.height - self.borderWidth);
    ...
    if (self.currentLayoutDirection == LeePopupContainerViewLayoutDirectionAbove) {
        // 让开，我要开始开始画三角形了，箭头向下
    ...
    if (self.currentLayoutDirection == LeePopupContainerViewLayoutDirectionBelow) {
        // 箭头向上
...
- (void)layoutDefaultSubviews {
    self.contentView.frame = CGRectMake(self.borderWidth + self.contentEdgeInsets.left, (self.currentLayoutDirection == LeePopupContainerViewLayoutDirectionAbove ? self.borderWidth : self.arrowSize.height + self.borderWidth) + self.contentEdgeInsets.top, CGRectGetWidth(self.bounds) - self.borderWidth * 2 - self.contentEdgeInsets.left - self.contentEdgeInsets.right, CGRectGetHeight(self.bounds) - self.arrowSize.height - self.borderWidth * 2 - self.contentEdgeInsets.top - self.contentEdgeInsets.bottom);
    ...
    if (isImageViewShowing) {
        ...
        _imageView.frame = CGRectMake(self.imageEdgeInsets.left, (CGRectGetHeight(self.contentView.bounds) - CGRectGetHeight(_imageView.frame))/2.0 + self.imageEdgeInsets.top, _imageView.frame.size.width, _imageView.frame.size.height);
    }
    if (isTextLabelShowing) {
        ...
        CGPoint textLabelOrigin = CGPointMake(textLabelMinX, (CGRectGetHeight(self.contentView.bounds)- ceil(textLabelSize.height))/2.0 + self.textEdgeInsets.top);
...
- (CGSize)contentSizeInSize:(CGSize)size {
    CGSize contentSize = CGSizeMake(size.width - self.contentEdgeInsets.left - self.contentEdgeInsets.right - self.borderWidth * 2, size.height - self.arrowSize.height - self.contentEdgeInsets.top - self.contentEdgeInsets.bottom - self.borderWidth * 2);
    return contentSize;
}

/// 根据内容大小和外部限制的大小，计算出合适的self size（包含箭头）
- (CGSize)sizeWithContentSize:(CGSize)contentSize sizeThatFits:(CGSize)sizeThatFits {
    CGFloat resultWidth = contentSize.width + self.contentEdgeInsets.left + self.contentEdgeInsets.right + self.borderWidth * 2;
    ...
    CGFloat resultHeight = contentSize.height + self.contentEdgeInsets.top + self.contentEdgeInsets.bottom + self.arrowSize.height + self.borderWidth * 2;
...
- (void)layoutWithTargetRect:(CGRect)targetRect inReferenceWindow:(UIWindow *)window {
    ...
    BOOL canShowAtAbove = [self canTipShowAtSpecifiedLayoutDirect:LeePopupContainerViewLayoutDirectionAbove targetRect:targetRect tipSize:tipSize];
    BOOL canShowAtBelow = [self canTipShowAtSpecifiedLayoutDirect:LeePopupContainerViewLayoutDirectionBelow targetRect:targetRect tipSize:tipSize];
    ...
    if (!canShowAtAbove && !canShowAtBelow) {
        ...
        _currentLayoutDirection = maximumHeightAbove > maximumHeightBelow ? LeePopupContainerViewLayoutDirectionAbove : LeePopupContainerViewLayoutDirectionBelow;
        
        NSLog(@"%@, 因为上下都不够空间，所以最大高度被强制改为%@, 位于目标的%@", self, @(self.maximumHeight), maximumHeightAbove > maximumHeightBelow ? @"上方" : @"下方");
        
    } else if (_currentLayoutDirection == LeePopupContainerViewLayoutDirectionAbove && !canShowAtAbove) {
        _currentLayoutDirection = LeePopupContainerViewLayoutDirectionBelow;
    } else if (_currentLayoutDirection == LeePopupContainerViewLayoutDirectionBelow && !canShowAtBelow) {
        _currentLayoutDirection = LeePopupContainerViewLayoutDirectionAbove;
    }
    ...
    if (_currentLayoutDirection == LeePopupContainerViewLayoutDirectionAbove) {
    ...
    } else if (_currentLayoutDirection == LeePopupContainerViewLayoutDirectionBelow) {
    ...
    self.frame = CGRectMake(tipMinX, tipMinY, tipSize.width, tipSize.height);
...
- (BOOL)canTipShowAtSpecifiedLayoutDirect:(LeePopupContainerViewLayoutDirection)direction targetRect:(CGRect)itemRect tipSize:(CGSize)tipSize {
    BOOL canShow = NO;
    CGFloat tipMinY = [self tipMinYWithTargetRect:itemRect tipSize:tipSize preferLayoutDirection:direction];
    if (direction == LeePopupContainerViewLayoutDirectionAbove) {
        canShow = tipMinY >= self.safetyMarginsOfSuperview.top;
    } else if (direction == LeePopupContainerViewLayoutDirectionBelow) {
        canShow = tipMinY + tipSize.height + self.safetyMarginsOfSuperview.bottom <= CGRectGetHeight(self.superviewIfExist.bounds);
    }
    return canShow;
}

- (CGFloat)tipMinYWithTargetRect:(CGRect)itemRect tipSize:(CGSize)tipSize preferLayoutDirection:(LeePopupContainerViewLayoutDirection)direction {
    CGFloat tipMinY = 0;
    if (direction == LeePopupContainerViewLayoutDirectionAbove) {
...
- (void)initPopupContainerViewWindowIfNeeded {
    if (!self.popupWindow) {
        self.popupWindow = [[LeePopupContainerViewWindow alloc] init];
        self.popupWindow.backgroundColor = [UIColor clearColor];
        self.popupWindow.windowLevel = UIWindowLevelAlert;
        LeePopContainerViewController *viewController = [[LeePopContainerViewController alloc] init];
        ((LeePopContainerMaskControl *)viewController.view).popupContainerView = self;
...
@implementation LeePopContainerMaskControl
...
@implementation LeePopupContainerView (UISubclassingHooks)
...
- (CGSize)sizeThatFitsInContentView:(CGSize)size {
    if (![self isSubviewShowing:_imageView]) {
        // 如果没内容则返回自身大小
...
@implementation LeePopupContainerView (UIAppearance)
...
+ (void)setDefaultAppearance {
    LeePopupContainerView *appearance = [LeePopupContainerView appearance];
    ...
    appearance.preferLayoutDirection = LeePopupContainerViewLayoutDirectionAbove;
    appearance.distanceBetweenTargetRect = 5;
    appearance.safetyMarginsOfSuperview = UIEdgeInsetsMake(10, 10, 10, 10);
    appearance.backgroundColor = [UIColor whiteColor];
    appearance.maskViewBackgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:.35f];
    appearance.highlightedBackgroundColor = nil;
    appearance.shadowColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:.1f];
    appearance.borderColor = [UIColor colorWithRed:198/255.0 green:198/255.0 blue:198/255.0 alpha:1];
    appearance.borderWidth = 1;
    appearance.cornerRadius = 10;
    appearance.lee_outsideEdge = UIEdgeInsetsZero;
...
- (void)updateAppearance {
    ...
    self.lee_outsideEdge = appearance.lee_outsideEdge;
}
