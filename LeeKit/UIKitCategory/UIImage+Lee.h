...
typedef NS_ENUM(NSInteger, LeeImageShape) {
    LeeImageShapeOval,                 // 椭圆
    LeeImageShapeTriangle,             // 三角形
    LeeImageShapeDisclosureIndicator,  // 列表 cell 右边的箭头
    LeeImageShapeCheckmark,            // 列表 cell 右边的checkmark
    LeeImageShapeDetailButtonImage,    // 列表 cell 右边的 i 按钮图片
    LeeImageShapeNavBack,              // 返回按钮的箭头
    LeeImageShapeNavClose              // 导航栏的关闭icon
};

typedef NS_OPTIONS(NSInteger, LeeImageBorderPosition) {
    LeeImageBorderPositionAll      = 0,
    LeeImageBorderPositionTop      = 1 << 0,
    LeeImageBorderPositionLeft     = 1 << 1,
    LeeImageBorderPositionBottom   = 1 << 2,
    LeeImageBorderPositionRight    = 1 << 3,
};
...
/**
 *  保持当前图片的形状不变，使用指定的颜色去重新渲染它，生成一张新图片并返回,图片是路径
 *
 ...
 *
 *  @return 处理完的图片
 *  @see lee_imageWithScaleToSize:contentMode:scale:
 */
