//
//  LeeModalPresentationViewController.m
//  LeeKit
//
...
@property(nonatomic, weak, readwrite) LeeModalPresentationViewController *lee_modalPresentationViewController;
@end

@implementation LeeModalPresentationViewController (UIAppearance)

static LeeModalPresentationViewController *appearance;
...
+ (void)initDefaultAppearance {
    ...
    appearance.animationStyle = LeeModalPresentationAnimationStyleFade;
    appearance.contentViewMargins = UIEdgeInsetsMake(20, 20, 20, 20);
    appearance.maximumContentViewWidth = ([UIScreen mainScreen].bounds.size.width - appearance.contentViewMargins.left - appearance.contentViewMargins.right);
}
...
@interface LeeModalPresentationViewController ()

@property(nonatomic, strong) LeeModalPresentationWindow *containerWindow;
...
@implementation LeeModalPresentationViewController
...
- (void)initDefaultDimmingViewWithoutAddToView {
    if (!self.dimmingView) {
        ...
        self.dimmingView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:.35f];
...
- (CGRect)contentViewFrameForShowing {
    CGSize contentViewContainerSize = CGSizeMake(CGRectGetWidth(self.view.bounds) - self.contentViewMargins.left - self.contentViewMargins.right, CGRectGetHeight(self.view.bounds) - self.keyboardHeight - self.contentViewMargins.top - self.contentViewMargins.bottom);
    ...
    CGRect contentViewFrame = CGRectMake((contentViewContainerSize.width - contentViewSize.width)/2.0 + self.contentViewMargins.left, (contentViewContainerSize.height - contentViewSize.height)/2.0 + self.contentViewMargins.top, contentViewSize.width, contentViewSize.height);
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.contentViewController) {
        self.contentViewController.lee_modalPresentationViewController = self;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    void (^didHiddenCompletion)(BOOL finished) = ^(BOOL finished) {
        ...
        if (self.contentViewController) {
            self.contentViewController.lee_modalPresentationViewController = nil;
...
- (void)setContentViewController:(UIViewController<LeeModalPresentationContentViewControllerProtocol> *)contentViewController {
...
- (void)showingAnimationWithCompletion:(void (^)(BOOL))completion {
    if (self.animationStyle == LeeModalPresentationAnimationStyleFade) {
    ...
    } else if (self.animationStyle == LeeModalPresentationAnimationStylePopup) {
        self.dimmingView.alpha = 0.0;
    ...
    } else if (self.animationStyle == LeeModalPresentationAnimationStyleSlide) {
        self.dimmingView.alpha = 0.0;
    ...
    }else if (self.animationStyle == LeeModalPresentationAnimationStyleTopInBottomOut) {
        self.dimmingView.alpha = 0.0;
...
- (void)showWithAnimated:(BOOL)animated completion:(void (^)(BOOL))completion {
    ...
    if (!self.containerWindow) {
        self.containerWindow = [[LeeModalPresentationWindow alloc] init];
...
- (void)hidingAnimationWithCompletion:(void (^)(BOOL))completion {
    ...
    } else if (self.animationStyle == LeeModalPresentationAnimationStyleSlide || self.animationStyle == LeeModalPresentationAnimationStyleTopInBottomOut) {
        [UIView animateWithDuration:.3 delay:0.0 options:7 << 16 animations:^{
            self.dimmingView.alpha = 0.0;
            self.contentView.transform = CGAffineTransformMakeTranslation(0, CGRectGetHeight(self.view.bounds) - CGRectGetMinY(self.contentView.frame));
        } completion:^(BOOL finished) {
...
- (void)handleKeyboardWillShow:(NSNotification *)notification {
    ...
    if (!self.view) {
        ...
        CGRect keyboardRectInView = [self.view convertRect:keyboardRect fromView:self.view.window];
        CGRect keyboardVisibleRectInView = CGRectIntersection(self.view.bounds, keyboardRectInView);
        keyboardHeight = CGRectIsNull(keyboardVisibleRectInView) ? 0.0f : CGRectGetHeight(keyboardVisibleRectInView);
    }
    if (keyboardHeight > 0) {
...
@implementation LeeModalPresentationWindow

@end

@implementation LeeModalPresentationViewController (Manager)

+ (BOOL)isAnyModalPresentationViewControllerVisible {
    for (UIWindow *window in [[UIApplication sharedApplication] windows]) {
        if ([window isKindOfClass:[LeeModalPresentationWindow class]] && !window.hidden) {
            return YES;
        }
    }
    return NO;
...
+ (BOOL)hideAllVisibleModalPresentationViewControllerIfCan {
    ...
    for (UIWindow *window in [[UIApplication sharedApplication] windows]) {
        if (![window isKindOfClass:[LeeModalPresentationWindow class]]) {
            continue;
        }
        ...
        LeeModalPresentationViewController *modalViewController = (LeeModalPresentationViewController *)window.rootViewController;
        BOOL canHide = YES;
...
@implementation UIViewController (LeeModalPresentationViewController)

static char kAssociatedObjectKey_ModalPresentationViewController;
- (void)setLee_modalPresentationViewController:(LeeModalPresentationViewController *)modalPresentedViewController {
...
- (LeeModalPresentationViewController *)lee_modalPresentationViewController {
    return (LeeModalPresentationViewController *)objc_getAssociatedObject(self, &kAssociatedObjectKey_ModalPresentationViewController);
}
