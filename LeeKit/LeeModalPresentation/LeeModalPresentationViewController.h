//
//  LeeModalPresentationViewController.h
//  LeeKit
//
...
@class LeeModalPresentationWindow;

typedef NS_ENUM(NSUInteger, LeeModalPresentationAnimationStyle) {
    LeeModalPresentationAnimationStyleFade,    // 渐现渐隐，默认
    LeeModalPresentationAnimationStylePopup,   // 从中心点弹出
    LeeModalPresentationAnimationStyleSlide,    // 从下往上升起
    LeeModalPresentationAnimationStyleTopInBottomOut //从上下来，从下出去
};

@protocol LeeModalPresentationContentViewControllerProtocol <NSObject>
 ...
 */
- (CGSize)preferredContentSizeInModalPresentationViewController:(LeeModalPresentationViewController *)controller limitSize:(CGSize)limitSize;

@end


@protocol LeeModalPresentationViewControllerDelegate <NSObject>
 ...
 */
- (BOOL)shouldHideModalPresentationViewController:(LeeModalPresentationViewController *)controller;

/**
 *  modalController 即将隐藏时的回调方法，在调用完这个方法后才开始做一些隐藏前的准备工作，例如恢复 window 的 dimmed 状态等。
 *  @param  controller  当前的modalController
 */
- (void)willHideModalPresentationViewController:(LeeModalPresentationViewController *)controller;
 ...
 *  @param  controller  当前的modalController
 */
- (void)didHideModalPresentationViewController:(LeeModalPresentationViewController *)controller;
 ...
 *  支持 3 种方式显示浮层：
 *
 *  1. **推荐** 新起一个 `UIWindow` 盖在当前界面上，将 `LeeModalPresentationViewController` 以 `rootViewController` 的形式显示出来，可通过 `supportedOrientationMask` 支持横竖屏，不支持在浮层不消失的情况下做界面切换（因为 window 会把背后的 controller 盖住，看不到界面切换）。
 ...
 *  @endcode
 *
 *  3. 将浮层作为一个 subview 添加到 `superview` 上，从而能够实现在浮层不消失的情况下进行界面切换，但需要 `superview` 自行管理浮层的大小和横竖屏旋转，而且 `LeeModalPresentationViewController` 不能用局部变量来保存，会在显示后被释放，需要自行 retain。横竖屏跟随当前界面的设置。
 ...
 *
 *  @see LeeAlertController
 *  @see LeeDialogViewController
 */

@interface LeeModalPresentationViewController : UIViewController {
    UITapGestureRecognizer      *_dimmingViewTapGestureRecognizer;
    CGFloat                     _keyboardHeight;
}

@property(nonatomic, weak) id<LeeModalPresentationViewControllerDelegate> delegate;
 ...
 */
@property(nonatomic, strong) IBOutlet UIViewController<LeeModalPresentationContentViewControllerProtocol> *contentViewController;
 ...
 *
 *  `LeeModalPresentationViewController`会自动给自定义的`dimmingView`添加手势以实现点击遮罩隐藏浮层。
 */
...
/**
 *  设置要使用的显示/隐藏动画的类型，默认为`LeeModalPresentationAnimationStyleFade`。
 *  @warning 当使用了`showingAnimation`和`hidingAnimation`时，该属性无效
 */
@property(nonatomic, assign) LeeModalPresentationAnimationStyle animationStyle UI_APPEARANCE_SELECTOR;
...
@interface LeeModalPresentationViewController (Manager)
...
@interface LeeModalPresentationViewController (UIAppearance)

+ (instancetype)appearance;

@end

/// 专用于LeeModalPresentationViewController的UIWindow，这样才能在`[[UIApplication sharedApplication] windows]`里方便地区分出来
@interface LeeModalPresentationWindow : UIWindow
