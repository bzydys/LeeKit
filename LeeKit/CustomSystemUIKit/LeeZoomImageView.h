...
@class LeeZoomImageView;
@class LeeEmptyView;
@class LeeButton;
@class LeeSlider;
@class LeeZoomImageViewVideoToolbar;
@class LeePieProgressView;

@protocol LeeZoomImageViewDelegate <NSObject>

@optional
- (void)singleTouchInZoomingImageView:(LeeZoomImageView *)zoomImageView location:(CGPoint)location;
- (void)doubleTouchInZoomingImageView:(LeeZoomImageView *)zoomImageView location:(CGPoint)location;
- (void)longPressInZoomingImageView:(LeeZoomImageView *)zoomImageView;
...
/// 是否支持缩放，默认为 YES
- (BOOL)enabledZoomViewInZoomImageView:(LeeZoomImageView *)zoomImageView;

// 可通过此方法调整视频播放时底部 toolbar 的视觉位置，默认为 {25, 25, 25, 18}
// 如果同时设置了 LeeZoomImageViewVideoToolbar 实例的 contentInsets 属性，则这里设置的值将不再生效
 ...
 *  @see viewportRect
 *
 *  LeeZoomImageView 提供最基础的图片预览和缩放功能以及 loading、错误等状态的展示支持，其他功能请通过继承来实现。
 */

@interface LeeZoomImageView : UIView <UIScrollViewDelegate>

@property(nonatomic, weak) id<LeeZoomImageViewDelegate> delegate;
...
// @see LeeZoomImageViewVideoToolbar
@property(nonatomic, strong, readonly) LeeZoomImageViewVideoToolbar *videoToolbar;

// 播放 video 时屏幕中央的播放按钮
@property(nonatomic, strong, readonly) LeeButton *videoCenteredPlayButton;

// 可通过此属性修改 video 播放时屏幕中央的播放按钮图片
@property(nonatomic, strong) UIImage *videoCenteredPlayButtonImage UI_APPEARANCE_SELECTOR;

// 从 iCloud 加载资源的进度展示
@property(nonatomic, strong) LeePieProgressView *cloudProgressView;

// 从 iCloud 加载资源失败的重试按钮
@property(nonatomic, strong) LeeButton *cloudDownloadRetryButton;

// 当前展示的资源的下载状态
@property(nonatomic, assign) LeeAssetDownloadStatus cloudDownloadStatus;
...
/**
 *  获取当前正在显示的图片/视频在整个 LeeZoomImageView 坐标系里的 rect（会按照当前的缩放状态来计算）
 */
...
@property(nonatomic, strong, readonly) LeeEmptyView *emptyView;
...
@interface LeeZoomImageViewVideoToolbar : UIView

@property(nonatomic, strong, readonly) LeeButton *playButton;
@property(nonatomic, strong, readonly) LeeButton *pauseButton;
@property(nonatomic, strong, readonly) LeeSlider *slider;
...
// 如果同时实现了 LeeZoomImageViewDelegate 的 contentInsetsForVideoToolbar:inZoomingImageView: 方法，则此处设置的值会覆盖掉 delegate 中返回的值
