...
typedef NS_ENUM(NSUInteger, LeeButtonImagePosition) {
    LeeButtonImagePositionTop,
    LeeButtonImagePositionLeft, //default
    LeeButtonImagePositionBottom,
    LeeButtonImagePositionRight,
};
 ...
 *  @warning LeeButton 重新定义了 UIButton.titleEdgeInests、imageEdgeInsets、contentEdgeInsets 这三者的布局逻辑，sizeThatFits: 里会把 titleEdgeInests 和 imageEdgeInsets 也考虑在内（UIButton 不会），以使这三个接口的使用更符合直觉。
 */
 ...
 *  @warning 不支持传 nil
 */
@property(nonatomic, strong, nonnull) UIColor *tintColorAdjustsTitleAndImage;

/**
 * 是否自动调整highlighted时的按钮样式，默认为YES
 * 当值为YES时，按钮highlighted时会改变自身的alpha属性为ButtonHighlightedAlpha
 */
...
/**
 * 设置按钮里图标和文字的相对位置，默认为LeeButtonImagePositionLeft
 * 可配合imageEdgeInsets、titleEdgeInsets、contentHorizontalAlignment、contentVerticalAlignment使用
 */
@property(nonatomic, assign) LeeButtonImagePosition imagePosition;
