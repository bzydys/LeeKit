...
@protocol LeeEmptyViewProtocol<NSObject>
...
/**
 *  通用的空界面控件，支持显示 loading、标题和副标题提示语、占位图片，LeeCommonViewController 内已集成一个 emptyView，无需额外添加。
 */

@interface LeeEmptyView : UIView

// 布局顺序从上到下依次为：imageView, loadingView, textLabel, detailTextLabel, actionButton
@property(nonatomic, strong) UIView<LeeEmptyViewProtocol> *loadingView;
...
/**
 *  如果要继承LeeEmptyView并添加新的子 view，则必须：
