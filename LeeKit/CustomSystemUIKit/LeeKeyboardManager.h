...
@class LeeKeyboardUserInfo;

/**
 *  `LeeKeyboardManagerDelegate`里面的方法是对应系统键盘通知的回调方法，具体请看delegate名字，`LeeKeyboardUserInfo`是对系统的userInfo做了一个封装，可以方便的获取userInfo的属性值。
 */
@protocol LeeKeyboardManagerDelegate <NSObject>
 ...
 */
- (void)keyboardWillChangeFrameWithUserInfo:(LeeKeyboardUserInfo *)keyboardUserInfo;

/**
 *  键盘已经显示
 */
- (void)keyboardDidShowWithUserInfo:(LeeKeyboardUserInfo *)keyboardUserInfo;

/**
 *  键盘已经隐藏
 */
- (void)keyboardDidHideWithUserInfo:(LeeKeyboardUserInfo *)keyboardUserInfo;

/**
 *  键盘frame已经发生变化。
 */
- (void)keyboardDidChangeFrameWithUserInfo:(LeeKeyboardUserInfo *)keyboardUserInfo;

@end


@interface LeeKeyboardManager : NSObject

/**
 *  指定初始化方法，以 delegate 的方式将键盘事件传递给监听者
 */
- (instancetype)initWithDelegate:(id<LeeKeyboardManagerDelegate>)delegate NS_DESIGNATED_INITIALIZER;

/**
 *  获取当前的 delegate
 */
@property(nonatomic, weak, readonly) id<LeeKeyboardManagerDelegate> delegate;
 ...
 */
+ (void)animateWithAnimated:(BOOL)animated keyboardUserInfo:(LeeKeyboardUserInfo *)keyboardUserInfo animations:(void (^)(void))animations completion:(void (^)(BOOL finished))completion;
...
@interface LeeKeyboardUserInfo: NSObject

/**
 *  所在的KeyboardManager
 */
@property(nonatomic, weak, readonly) LeeKeyboardManager *keyboardManager;
