...
/// 相册展示内容的类型
typedef NS_ENUM(NSUInteger, LeeAlbumContentType) {
    LeeAlbumContentTypeAll,                                  // 展示所有资源
    LeeAlbumContentTypeOnlyPhoto,                            // 只展示照片
    LeeAlbumContentTypeOnlyVideo,                            // 只展示视频
    LeeAlbumContentTypeOnlyAudio                             // 只展示音频
};

/// 相册展示内容按日期排序的方式
typedef NS_ENUM(NSUInteger, LeeAlbumSortType) {
    LeeAlbumSortTypePositive,  // 日期最新的内容排在后面
    LeeAlbumSortTypeReverse  // 日期最新的内容排在前面
};

@interface LeeAssetsGroup : NSObject
 ...
 *                          可以以此作为判断枚举结束的标记
 */
- (void)enumerateAssetsUsingBlock:(void (^)(LeeAsset *result))enumerationBlock;
