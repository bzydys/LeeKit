...
void LeeImageWriteToSavedPhotosAlbumWithAlbumAssetsGroup(UIImage *image, LeeAssetsGroup *albumAssetsGroup, LeeWriteAssetCompletionBlock completionBlock) {
    [[LeeAssetsManager sharedInstance] saveImageWithImageRef:image.CGImage albumAssetsGroup:albumAssetsGroup orientation:image.imageOrientation completionBlock:completionBlock];
}

void LeeSaveImageAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *imagePath, LeeAssetsGroup *albumAssetsGroup, LeeWriteAssetCompletionBlock completionBlock) {
    [[LeeAssetsManager sharedInstance] saveImageWithImagePathURL:[NSURL fileURLWithPath:imagePath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}

void LeeSaveVideoAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *videoPath, LeeAssetsGroup *albumAssetsGroup, LeeWriteAssetCompletionBlock completionBlock) {
    [[LeeAssetsManager sharedInstance] saveVideoWithVideoPathURL:[NSURL fileURLWithPath:videoPath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}

@implementation LeeAssetsManager{
    PHCachingImageManager *_phCachingImageManager;
}

+ (LeeAssetsManager *)sharedInstance {
    static dispatch_once_t onceToken;
    static LeeAssetsManager *instance = nil;
...
+ (LeeAssetAuthorizationStatus)authorizationStatus {
    __block LeeAssetAuthorizationStatus status;
    // 获取当前应用对照片的访问授权状态
    PHAuthorizationStatus authorizationStatus = [PHPhotoLibrary authorizationStatus];
    if (authorizationStatus == PHAuthorizationStatusRestricted || authorizationStatus == PHAuthorizationStatusDenied) {
        status = LeeAssetAuthorizationStatusNotAuthorized;
    } else if (authorizationStatus == PHAuthorizationStatusNotDetermined) {
        status = LeeAssetAuthorizationStatusNotDetermined;
    } else {
        status = LeeAssetAuthorizationStatusAuthorized;
    }
    return status;
}

+ (void)requestAuthorization:(void(^)(LeeAssetAuthorizationStatus status))handler {
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus phStatus) {
        LeeAssetAuthorizationStatus status;
...
- (void)enumerateAllAlbumsWithAlbumContentType:(LeeAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbumIfSupported:(BOOL)showSmartAlbumIfSupported usingBlock:(void (^)(LeeAssetsGroup *resultAssetsGroup))enumerationBlock {
    ...
    // 创建一个 PHFetchOptions，用于 LeeAssetsGroup 对资源的排序以及对内容类型进行控制
    PHFetchOptions *phFetchOptions = [PHPhotoLibrary createFetchOptionsWithAlbumContentType:contentType];
    
    // 遍历结果，生成对应的 LeeAssetsGroup，并调用 enumerationBlock
    for (NSUInteger i = 0; i < [tempAlbumsArray count]; i++) {
        PHAssetCollection *phAssetCollection = [tempAlbumsArray objectAtIndex:i];
        LeeAssetsGroup *assetsGroup = [[LeeAssetsGroup alloc] initWithPHCollection:phAssetCollection fetchAssetsOptions:phFetchOptions];
        if (enumerationBlock) {
...
- (void)enumerateAllAlbumsWithAlbumContentType:(LeeAlbumContentType)contentType usingBlock:(void (^)(LeeAssetsGroup *resultAssetsGroup))enumerationBlock {
...
- (void)saveImageWithImageRef:(CGImageRef)imageRef albumAssetsGroup:(LeeAssetsGroup *)albumAssetsGroup orientation:(UIImageOrientation)orientation completionBlock:(LeeWriteAssetCompletionBlock)completionBlock {
...
- (void)saveImageWithImagePathURL:(NSURL *)imagePathURL albumAssetsGroup:(LeeAssetsGroup *)albumAssetsGroup completionBlock:(LeeWriteAssetCompletionBlock)completionBlock {
    PHAssetCollection *albumPhAssetCollection = albumAssetsGroup.phAssetCollection;
    // 把图片加入到指定的相册对应的 PHAssetCollection
...
- (void)saveVideoWithVideoPathURL:(NSURL *)videoPathURL albumAssetsGroup:(LeeAssetsGroup *)albumAssetsGroup completionBlock:(LeeWriteAssetCompletionBlock)completionBlock {
    PHAssetCollection *albumPhAssetCollection = albumAssetsGroup.phAssetCollection;
...
@implementation PHPhotoLibrary (Lee)

+ (PHFetchOptions *)createFetchOptionsWithAlbumContentType:(LeeAlbumContentType)contentType {
    PHFetchOptions *fetchOptions = [[PHFetchOptions alloc] init];
    // 根据输入的内容类型过滤相册内的资源
    switch (contentType) {
        case LeeAlbumContentTypeOnlyPhoto:
            fetchOptions.predicate = [NSPredicate predicateWithFormat:@"mediaType = %i", PHAssetMediaTypeImage];
            break;
            
        case LeeAlbumContentTypeOnlyVideo:
            fetchOptions.predicate = [NSPredicate predicateWithFormat:@"mediaType = %i",PHAssetMediaTypeVideo];
            break;
            
        case LeeAlbumContentTypeOnlyAudio:
...
+ (NSArray *)fetchAllAlbumsWithAlbumContentType:(LeeAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbum:(BOOL)showSmartAlbum {
    NSMutableArray *tempAlbumsArray = [[NSMutableArray alloc] init];
    
    // 创建一个 PHFetchOptions，用于创建 LeeAssetsGroup 对资源的排序和类型进行控制
