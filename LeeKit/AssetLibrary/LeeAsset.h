...
/// Asset 的类型
typedef NS_ENUM(NSUInteger, LeeAssetType) {
    LeeAssetTypeUnknow,                                    // 未知类型的 Asset
    LeeAssetTypeImage,                                     // 图片类型的 Asset
    LeeAssetTypeVideo,                                     // 视频类型的 Asset
    LeeAssetTypeAudio                                      // 音频类型的 Asset
};

typedef NS_ENUM(NSUInteger, LeeAssetSubType) {
    LeeAssetSubTypeUnknow,                                 // 未知类型
    LeeAssetSubTypeImage,                                  // 静态图片类型
    LeeAssetSubTypeLivePhoto NS_ENUM_AVAILABLE_IOS(9_1),   // Live Photo 类型
    LeeAssetSubTypeGIF                                     // GIF类型
};

/// 从 iCloud 请求 Asset 大图的状态
typedef NS_ENUM(NSUInteger, LeeAssetDownloadStatus) {
    LeeAssetDownloadStatusSucceed,     // 下载成功或资源本来已经在本地
    LeeAssetDownloadStatusDownloading, // 下载中
    LeeAssetDownloadStatusCanceled,    // 取消下载
    LeeAssetDownloadStatusFailed,      // 下载失败
};

@class PHAsset;


@interface LeeAsset : NSObject

@property(nonatomic, assign, readonly) LeeAssetType assetType;
@property(nonatomic, assign, readonly) LeeAssetSubType assetSubType;

- (instancetype)initWithPHAsset:(PHAsset *)phAsset;

@property(nonatomic, strong, readonly) PHAsset *phAsset;
@property(nonatomic, assign, readonly) LeeAssetDownloadStatus downloadStatus; // 从 iCloud 下载资源大图的状态
 ...
 *
 *  @param completion        完成请求后调用的 block，参数中包含了请求的 Live Photo 以及相关信息，若 assetType 不是 LeeAssetTypeLivePhoto 则为 nil
 *  @param phProgressHandler 处理请求进度的 handler，不在主线程上执行，在 block 中修改 UI 时注意需要手工放到主线程处理。
 *
 ...
 *
 *  @param completion        完成请求后调用的 block，参数中包含了请求的 AVPlayerItem 以及相关信息，若 assetType 不是 LeeAssetTypeVideo 则为 nil
 *  @param phProgressHandler 处理请求进度的 handler，不在主线程上执行，在 block 中修改 UI 时注意需要手工放到主线程处理。
 *
 ...
 *  异步请求图片的 Data
 *
 *  @param completion 完成请求后调用的 block，参数中包含了请求的图片 Data（若 assetType 不是 LeeAssetTypeImage 或 LeeAssetTypeLivePhoto 则为 nil），该图片是否为 GIF 的判断值，以及该图片的文件格式是否为 HEIC
 */
- (void)requestImageData:(void (^)(NSData *imageData, NSDictionary<NSString *, id> *info, BOOL isGIF, BOOL isHEIC))completion;

/**
 * 获取图片的 UIImageOrientation 值，仅 assetType 为 LeeAssetTypeImage 或 LeeAssetTypeLivePhoto 时有效
 */
- (UIImageOrientation)imageOrientation;

/**
 *  Asset 的标识，每个 LeeAsset 的标识值不相同，该标识值经过 md5 处理，避免了特殊字符
 *
 *  @return Asset 的标识字符串
 */
