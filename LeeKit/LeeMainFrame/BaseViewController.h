 ...
 *
 *  1. 自带空界面控件 `LeeEmptyView`，支持显示loading、空文案、操作按钮
 *
 *  2. 自动在 `dealloc` 时移除所有注册到 `NSNotificationCenter` 里的监听，避免野指针 crash
 *
 *  3. 统一约定的常用接口，例如初始化 subview、设置顶部 `navigationItem`，从而保证相同类型的代码集中到同一个方法内，避免多人交叉维护时代码分散难以查找
 *
 *  4. 配合 `BaseNavigationController` 使用时，可以得到 `willPopInNavigationControllerWithAnimated:`、`didPopInNavigationControllerWithAnimated:` 这两个时机
 *  @see LeeEmptyView
 */
 ...
 */
@property(nonatomic, strong) LeeEmptyView *emptyView;
...
@interface BaseViewController (UISubclassingHooks)
...
/**
 *  为了方便实现“点击空白区域降下键盘”的需求，BaseViewController 内部集成一个 tap 手势对象并添加到 self.view 上，而业务只需要通过重写 -shouldHideKeyboardWhenTouchInView: 方法并根据当前被点击的 view 返回一个 BOOL 来控制键盘的显隐即可。
 *  @note 为了避免不必要的事件拦截，集成的手势 hideKeyboardTapGestureRecognizer：
 *  1. 默认的 enabled = NO。
 *  2. 如果当前 viewController 或其父类（非 BaseViewController 那个层级的父类）没重写 -shouldHideKeyboardWhenTouchInView:，则永远 enabled = NO。
...
@property(nonatomic, strong, readonly) LeeKeyboardManager *hideKeyboardManager;
